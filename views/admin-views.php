...
if ( !class_exists( 'ABD_Admin_Views' ) ) {
	class ABD_Admin_Views {
		...
		public static function wpsm_settings() {
			...
			new ABDWPSM_Tab( array(
				'display_name'        => ABD_L::__( 'Admiral Settings' ),
				'display_description' => self::admiral_settings_tab_header(),
				'url_slug'            => 'admiral-settings',
				'page'                => 'ad-blocking-detector'
			) );
		...
		protected static function admiral_settings_tab_header() {
			ob_start();
			\abd\AdmiralAdBlockAnalytics::registerSettings('ad-blocking-detector-settings-group');
			?>

			<h2><?php ABD_L::_e( 'Admiral Settings' ); ?></h2>

			<?php
			echo self::admiral_settings_tab_content();

			return ob_get_clean();
		}

		protected static function admiral_settings_tab_content() {
			?>

			<p style="font-size: 14px; margin-bottom: 10px; max-width: 750px;">Ad Blocking Detector is now integrated with Admiral Adblock Suite. Admiral provides the most advanced adblock analytics platform in the industry and offers premium products like <a href="https://getadmiral.com/products/engage">Engage</a> and <a href="https://getadmiral.com/products/transact">Transact</a> that empower you to recover more revenue than ever before. <a href="https://getadmiral.com/">Learn More</a></p>
			<?php echo \abd\AdmiralAdBlockAnalytics::getCTA(); ?>
			<form method="post" action="options.php">
				<?php settings_fields( 'ad-blocking-detector-settings-group' ); ?>
				<?php do_settings_sections( 'ad-blocking-detector-settings-group' ); ?>
				<?php echo \abd\AdmiralAdBlockAnalytics::getPropertyIDInput(); ?>
				<?php submit_button(); ?>
			</form>
			<?php
		}
